...
- 自律監視は、保存された設定ID・有効状態・対象・次回実行予定を確認して初めて「登録確認済み」、初回実行の結果まで確認して初めて「稼働確認済み」と報告する。確認できない項目は明記する。担当停止・実行失敗・再開不能は「変化なし・通知不要」に含めず、検知した実行で復旧を試み、未完了の停止はユーザーへ知らせる。
...
- 親PM: `gpt-6-astra`。要件、範囲、受入条件、依存判断、証拠検証、品質ゲート、進行管理、次指示を担当する。
