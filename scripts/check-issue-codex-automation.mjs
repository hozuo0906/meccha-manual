...
for (const path of requiredFiles) await read(path);
...
for (const token of [
  ...
  "GH_TOKEN: \"\"", "GITHUB_TOKEN: \"\"", "run_untrusted run check", "--script-shell=/bin/bash", "Prepare writable isolated candidate copy", "git diff --check", "verify-head",
...
if (/^concurrency:\s*$/m.test(codexWorkflow)) errors.push("Codex issue implement workflow must not define root-level concurrency.");
...
if (!implementMarker) {
  ...
  if (!concurrencyMatch) {
    ...
    if (!groupMatch || groupMatch[1].trim() !== "codex-issue-implement") errors.push("Codex issue implement job must use the repository-wide issue implementation concurrency group.");
    if (!/^      cancel-in-progress:\s+false\s*$/m.test(concurrency)) errors.push("Codex issue implement job must keep cancel-in-progress false.");
    if (!/^      queue:\s+max\s*$/m.test(concurrency)) errors.push("Codex issue implement job must use the standard max concurrency queue.");
  }
}
...
if (intakeReport.includes("ユーザー承認なしに実装、merge、本番deployしない")) errors.push("Issue intake report must not use the superseded blanket approval wording.");
...
if (workerSource.includes("owner承認を確認してから行います") || workerSource.includes("レビュー、owner承認を確認してからmergeします")) errors.push("Worker merge guidance must not use the superseded blanket owner-approval wording.");
