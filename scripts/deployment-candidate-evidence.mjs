...
if (mode === "verify-candidate") {
  if (git("rev-parse", "HEAD") !== candidate) throw new Error("Checked out HEAD differs from CANDIDATE_SHA.");
  ...
  console.log("Deployment candidate is an immutable commit reachable from main.");
  ...
  if (evidence.candidateSha !== candidate || evidence.repository !== process.env.GITHUB_REPOSITORY || evidence.runId !== runId) {
  ...
  if (run.path !== ".github/workflows/deploy-staging.yml" || run.event !== "workflow_dispatch" || run.conclusion !== "success") {
