...
definitions = replaceOnce(
  ...
  "| `manuals` | `workspace_id`, `folder_id`, `title`, `status`, `current_draft_revision_id`, `current_published_revision_id`, `owner_id`, `archived_at` | メンバー閲覧。作成・draft metadata・公開状態変更はSECURITY DEFINER RPCのみ。raw `title`は1〜64文字、ECMAScript `trim()`相当後に空でないことを`manuals_title_length` / `manuals_title_nonblank`で強制し、authenticated direct writeをrevoke |",
...
definitions = replaceOnce(
  ...
  "| `manual_revisions` | `workspace_id`, `manual_id`, `revision_no`, `state`, `title`, `description`, `source_url`, `cover_asset_id`, `published_at` | メンバー閲覧、公開版は不変。draft更新・作成・公開はRPCのみ。titleは`manual_revisions_title_length` / `manual_revisions_title_nonblank`、descriptionは`manual_revisions_description_length`で強制し、authenticated direct writeをrevoke |",
