...
function hasAiRuntimeBoundary(content, path = "") {
  ...
  const hasLegacySqlSetConfig = normalizedPath.endsWith(".sql") && /\bset_config\s*\(\s*'standard_conforming_strings'\s*,\s*'off'/i.test(sqlLexicalContent);
  ...
  const hasUnapprovedOutboundCapability = (
    ...
    || hasDynamicSqlExecute || hasSqlOutboundToken || hasSqlUnicodeEscapedIdentifier || hasSqlUnicodeEscapedString || hasSqlNumericEscape || hasLegacySqlBackslashEscapes || hasLegacySqlSetConfig
  );
  return (
