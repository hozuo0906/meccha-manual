...
function hasAiRuntimeBoundary(content, path = "") {
  ...
  const hasRefreshNavigationSink = /\bheaders\s*:\s*\{[^}]*?(?:["']?refresh["']?|\[\s*["']refresh["']\s*\])\s*:/i.test(normalizedContent)
    || /\.(?:set|append)\s*\(\s*["']refresh["']/i.test(normalizedContent);
