...
function hasAiRuntimeBoundary(content, path = "") {
  ...
  const hasUnapprovedOutboundCapability = (
    ...
    || /\bpg_net\b|\bnet\s*\.\s*http_(?:get|post|delete|head)\b|\bsupabase_functions\s*\.\s*http_request\b|\bextensions\s*\.\s*http(?:_(?:get|post|put|delete|head))?\b|\bhttp_(?:get|post|put|delete|head)\s*\(/i.test(capabilityContent)
  );
  return (
