...
- サーバー内部の保存要求metadataは `workspace_id`、object keyの第3要素と一致する `resource_id`、`asset_id`、任意の `generation_id`、`reservation_id`、`fencing_token`、`manual_id`、`step_id` だけを受け付ける。予約IDとfencing tokenは必ず対で、予約世代がある場合だけ許可する。`resource_id`と`generation_id`はkey検証用の一時値でPostgres/R2へ汎用列として保存しない。R2 custom metadataへは予約objectの `reservation_id` と `fencing_token` だけを世代照合用に複製し、`resource_id`、`generation_id`、`manual_id`、`step_id` は複製しない。read時のresourceと予約世代は検証済みobject keyから導出する。
