...
- サーバー内部のmetadataは `workspace_id`、object keyの第3要素と一致する `resource_id`、`asset_id`、任意の `manual_id`、`step_id` だけを受け付ける。R2 custom metadataへは `resource_id`、`manual_id`、`step_id` を複製せず、read時のresourceは検証済みobject keyから導出する。
