...
|---|---|---|---|---|---|---|
| FR-001 | SCR-LOGIN, SCR-SHELL | Access callback/JWT検証、`GET /api/session`、Access logout／再認証導線 | identities, profiles | ADR-0028 | AC-001, AC-003, AC-004, AC-005、`tests/access-identity.test.mjs`（M1 verifier／actor／identity DI）、`tests/m3-http-d1.test.mjs`（session／logout HTTP path）、`tests/app-auth.test.mjs`（Access JWT終端401の共有version通知、兄弟タブの遅着破棄・再通知抑止、403主体拒否、503一時障害、Access logout分類） | #176, EPIC-02, EPIC-03 |
