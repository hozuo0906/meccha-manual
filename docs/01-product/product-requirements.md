...
認証前eventはlocal-onlyとし、認証せず離脱したguestをanalytics目的でuploadしない。このためFull Guest Signup Conversion、Full Guest Activation Rate、Full Guest Output Gate Conversionは現行MVPのserver-side Product Eventから算出不可とし、認証後にflushしたcreator cohortの診断値と区別する。
