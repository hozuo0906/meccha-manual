...
- AI拡張は売上が安定しownerが明示承認するまで設計・実装・設定を開始しない。
...
| FR-020 | 将来のAI拡張を承認ゲートで管理する | 売上安定とowner明示承認の両方が揃うまでadapter、flag、Secret、endpointを作らず、外部AI APIを呼ばない |
...
- Browser Run開始前に残り時間を原子的に予約する。close不能時も予約期限でremote sessionが停止する絶対失効機構をP0実証できるまでは起動をfail closedにする。
- R2書込前に現在量と予定byteを原子的に予約する。object不在が確定した失敗だけ即時解放し、結果不明はreconciliation完了まで予約を保持する。
