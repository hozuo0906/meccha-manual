...
- D1の`workspace_kind = 'personal'`とstatusを問わない`UNIQUE(created_by) WHERE workspace_kind = 'personal'`を一意境界とし、既存standard workspaceをPersonalと推測しない。並行bootstrapはunique conflict後に同じactive Personal Workspaceを再読込して同じworkspaceIdへ収束する。既存Personal Workspaceがsuspended／deletedなら2件目を作らず`PERSONAL_WORKSPACE_UNAVAILABLE`でfail closedにする。
