...
- 同じ予約を複数schedulerが同時にreconcileしても、外部I/O後に予約stateをlock／CAS相当で再確認し、確定と`current bytes`加算を一度だけ行う。期限切れ解放側も削除I/O後にstateを再確認し、並行確定を`released`で上書きしない。削除開始前に対象object keyとcommit禁止のrelease-only遷移を永続化し、削除応答を失った場合は破壊的I/O pendingと回収記録を保持する。claim期限後は再起動したreconcilerが同じkeyの削除を冪等に再実行し、release-only予約を確定へ戻さず解放へ収束させる。これにより遅延削除後のobject欠落と容量計上の不一致、および中断claimによる予約枠の永久保持を防ぐ。
