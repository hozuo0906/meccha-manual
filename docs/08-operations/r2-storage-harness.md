...
- reconciliation jobはactive予約と、released／committed予約ごとの永続generation tombstoneを定期走査し、停止・再起動後も再開する。固定時間では打ち切らず、60秒を超えて遅着するPUTも同世代prefixから回収する。tombstoneを走査集合から外せるのは、R2 lifecycle ruleがそのgeneration prefixの全objectを削除済みであり、以後その世代へ書込みを開始できないことをprovider証跡で確認した後だけとする。archive遷移はprovider verifierが返すlifecycle削除済み、write fencing済み、provider evidence ID、generation ID、generation prefixを予約と照合して原子的に保存する。別世代・別prefixの証跡は拒否し、同じ証跡の再送だけを冪等に受理して、最初の証跡tupleと時刻を上書きしない。確認済みtombstoneをactive sweepから外してLISTを累積履歴へ比例させない。予約単位で失敗を記録・再試行し、1件の失敗で後続予約の走査を中断しない。R2本文からchecksumを再計算し、workspace、key、size、checksum、予約ID、fencing tokenが一致したobjectだけを確定する。
