...
2. Ownerが承認したjobだけをclaimし、agent jobでbase branchをcommit SHAへ固定する。既存publication branchがある新規retry jobでは、そのbranchの単一親commitを復元し、現在のbase branchの祖先であることを確認する。testとpublishは同じSHAをcheckoutする。
