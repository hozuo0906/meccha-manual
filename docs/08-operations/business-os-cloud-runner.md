...
2. Ownerが承認したjobだけをclaimし、agent jobでbase branchをcommit SHAへ固定する。testとpublishは同じSHAをcheckoutする。
...
9. event IDはjob・type・sequenceから決定的に生成し、workflow再実行でも同じtransitionを冪等に返す。agent内でclaim後に失敗した場合は、localのtrusted stateからacceptedとfailure監査eventを返すため、state artifact upload失敗だけで監査を失わない。
