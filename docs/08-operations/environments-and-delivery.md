...
| Legacy RLS immutable preview | 移行前baseline。新規実行しない | 使用しない | 旧Supabase workflowをdispatchせず、test user・資格情報を追加しない。Access/D1/R2の代替gateはIssue #176 M5で実施する |
...
| Legacy immutable preview gate | 実行しない | Superseded。旧Supabase workflow、test user、資格情報を追加せず、Issue #176 M5のAccess/D1/R2実preview gateへ置換する |
