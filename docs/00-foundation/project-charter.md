...
- Full Guest Activation Rate: 現行MVPのserver-side Product Eventでは算出不可。認証後にeventをflushしたcreator cohortの診断値をProduct全体のActivation Rateとして公開しない。
- TTFV: `capture_started` からlocal `draft_generated`までを主指標とする。server-side集計は後に認証してprivacy-safe eventをflushしたcohortに限定し、未認証の全guestを完全観測できるとは扱わない。
- Capture Completion／Failure: 認証後にeventをflushしたcreator cohort内の診断値だけを算出でき、unbiasedなfull-guest rateとは扱わない。
- Full Guest Output Gate Conversion: 現行MVPのserver-side Product Eventでは算出不可。
- Full Guest Signup Conversion: 現行MVPのserver-side Product Eventでは算出不可。pre-auth匿名trackingは追加しない。
