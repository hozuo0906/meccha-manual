...
- Activation Rate: output gateへ到達したguest cohortのうち最初のmanualをserver側に確定できた割合。returning／既存認証済みcreatorは含めない。
...
- Signup Conversion: 新規アカウント作成を開始した利用者のうち、server-sideでapplication identityが新規作成された割合。returning userのloginは除外する。
