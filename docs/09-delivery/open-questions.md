...
| OQ-027 | TeamからPersonalへ移行するとき、既存のowner以外のメンバーをどう扱うか | 自動削除・自動降格は行わない。決定までは有効メンバーが2人以上のworkspaceでPersonalのCheckout Session作成を拒否し、課金前に移行を止める | pricing/checkout/membership |
