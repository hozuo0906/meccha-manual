...
| OQ-029 | Accessの招待照合、subject再発行、email変更、退会後再登録をどう扱うか | Accepted (M1): 検証済みissuer+subjectをidentityの正本とし、email変更ではidentityを移動しない。subject再発行は未知identityとして扱い、退会・disabled identityを自動復活させない。bootstrap／relinkは初期無効にし、email一致だけでidentityやmembershipを作成しない。D1制約、招待digestの期限・単回消費・atomic membership、運用provisioningはM2、OTP／招待UIはM3で扱う | Access identity spike / 外部招待 |
