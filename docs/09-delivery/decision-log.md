...
| DEC-043 | 2026-08-10 | 現在workspaceはuser IDと最新active所属で検証したタブ内選択として保持し、認可根拠にしない。作成RPCの応答消失・不正成功応答は確定失敗とせず結果不明として一覧確認を案内する（[ADR-0010](../03-architecture/adrs/ADR-0010-worker-cookie-auth-harness.md)） | 共有ブラウザでの別ユーザー選択持越しと、作成済みworkspaceの重複再作成を防ぐため |
