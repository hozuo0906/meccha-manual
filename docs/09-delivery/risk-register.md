...
| RISK-019 | 並行開始・保存でBrowser Run時間またはR2容量を上限超過する | 原価超過/保存失敗 | 利用前に原子的予約し、成功時に確定する。confirmed non-start/nonexistenceだけ即時解放し、result-unknownはreconciliationまで保持する。expiredでobject不在でもoriginal writerのterminal/fenced（lease generation/fencingまたはprovider-terminal proof）を確認するまで解放せず、Browser Runはhard deadlineで終了する |
