...
- SQLの同一statementで `COPY` の `TO`／`FROM` 直後にある `PROGRAM` だけを拒否し、同名columnや無関係な後続statementを誤検知せず、DB server上のOS command経由で外部通信を開始できないようにする。
- 静的HTMLを構造解析し、script等のactive content、event handler、meta refresh、未承認host／protocol-relativeのresource URL、CSS URL／escapeを拒否する。resource URLは実配信originをbaseにWHATWG正規化した後で判定し、既存の相対URLと承認済みHTTPS resourceだけを許可する。
