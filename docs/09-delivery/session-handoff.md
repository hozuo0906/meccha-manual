...
- 承認前runtimeでは`sendBeacon` capability名を表記に関係なく拒否し、optional chaining、computed member、分割代入、prototype呼出しで外向きPOSTを隠せないようにする。動的keyを使うcomputed分割代入はJavaScript構造tokenで括弧深度を追跡し、ネストした添字式でも`navigator`／`Navigator.prototype`からのcapability取得として拒否する。
