...
- 完了予約は永続generation tombstoneとして再走査し、60秒を超える遅着PUTも回収する。R2 lifecycleによる全object削除と同世代への新規書込不能をprovider証跡で確認した後だけ、証跡ID付きの原子的archive遷移でactive走査集合から外し、LISTを累積履歴へ比例させないfixtureを追加した。
