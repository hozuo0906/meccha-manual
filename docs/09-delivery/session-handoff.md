...
- migrationで許可する手続き言語を `sql`／`plpgsql` に固定し、通常identifier・通常文字列・E文字列・dollar文字列のLANGUAGE値を構造tokenで照合する。`CREATE [OR REPLACE] [TRUSTED] [PROCEDURAL] LANGUAGE`／`ALTER`／`DROP LANGUAGE`による許可名handlerの差し替えと、plperl等の未承認言語、`spi_*` 能力を拒否する。末尾column aliasの `language` は値を持たないため誤検知しない。
