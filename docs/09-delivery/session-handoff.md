...
- migrationで許可する手続き言語を `sql`／`plpgsql` に固定し、通常identifier・通常文字列・E文字列・dollar文字列のLANGUAGE値を構造tokenで照合する。plperl等の未承認言語と `spi_*` 能力を拒否し、文字列内へserver programを隠せないようにする。
