...
- generatorの`yield`によるconfig owner escapeはコメントを挟む表記もtoken単位で拒否する。Responseの`Report-To`／`NEL`／`Reporting-Endpoints`とCSP／CSP Report-Onlyの`report-uri`／`report-to`をbrowser outbound header sinkとして拒否する。
