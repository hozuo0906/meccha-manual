...
- generatorの`yield`によるconfig owner escapeはコメントを挟む表記もtoken単位で拒否する。Responseの`Report-To`／`NEL`／`Reporting-Endpoints`をbrowser outbound header sinkとして拒否する。CSPは`apps/worker/src/index.ts`の固定`SECURITY_HEADERS`宣言だけを許可し、CSP Report-Only、alias／computed policyを含むそれ以外のCSP header capabilityは値を追跡せずfail closedで拒否する。
