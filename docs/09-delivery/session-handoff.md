...
- 正規化: click、input completion、navigation、scrollだけを最大200件受理し、正の一意sequence順へ整列する。重複sequenceは先勝ちにせず、同じsequenceを持つ全eventを除外する。input completionのtargetは入力値由来でないことを証明できないため常に`入力欄`へ置換する。未知field、入力値、URL資格情報・query・fragment、Cookie、Authorizationは出力へ複製しない。
