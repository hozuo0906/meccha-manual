...
- 終了処理
- SSRF対策
- 月次残り時間の原子的予約、hard deadline、cancel/close
...
- マスキング
- 下書き生成
- R2予定byteの原子的予約。成功時に確定し、object不在が確定した失敗だけ解放、結果不明はreconciliationまで保持
...
- 請求・利用量画面
- Browser Run時間とR2容量の予約・確定・解放をserver側で強制
...
- 売上安定とowner明示承認が揃うまで着手しない
- 承認後に別ADRで送信範囲、上限、監査、停止条件を決定
