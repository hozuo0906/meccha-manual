...
- iframe: Deferred。Markdown／HTML: NEXT。
- PDF runtime実装はFR-014／MVP-AC-019を完了条件とし、guestのPDF選択からsignup／bootstrap／claim後の自動再開、取消時のlocal原本保持、claim成功前の削除禁止を同じ実装Issueで検証する。
