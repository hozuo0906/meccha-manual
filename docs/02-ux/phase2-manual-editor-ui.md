...
- 作成結果不明時は自動再送せず、作成元workspace単位のメモリ状態として保持する。別workspaceへ移動して戻った場合も、作成元の一覧を再取得して「重ねて作成しない」案内を表示する。
- 作成結果不明より前に開始した一覧取得が後から完了しても警告を消さず、結果確認用の新しい一覧取得が完了するまで作成元workspaceを未照合状態に保つ。
...
- 編集mutationが403または所属喪失を示す404になった場合は編集UIを安全側で閉じる。同時に共有のワークスペース権限キャッシュも破棄・再取得し、一覧へ戻った際に作成UIが復活しないようにする。
...
- owner/admin/editorには、draftがある場合だけ確認付きの「この内容を公開」を表示する。未保存フォームがある間は拒否し、公開版が直接変更できないこと、機密情報がなく必要なマスキングが完了したことを明示確認する。
...
- 403権限失効・所属喪失404時の編集UI閉鎖、共有ロールキャッシュ破棄、一覧へ戻った後の作成UI非表示
