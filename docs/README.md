...
商用MVPや新機能の優先順位を判断するときは、次を先に確認します。

1. [プロジェクト憲章](00-foundation/project-charter.md) — ICP、Job To Be Done、guest-first Activation、Product KPI。
2. [スコープ](01-product/scope.md) — MVP / NEXT / Deferred、3表示モード。
3. [機能要件](01-product/product-requirements.md) — FRごとのLaunch優先度。
4. [UX原則](02-ux/experience-principles.md) — TTFV、Progressive Disclosure、output gate。
5. [オンボーディング](02-ux/onboarding.md) — guest作成からoutput時signupまでの正本。
6. [Chrome拡張only ADR](03-architecture/adrs/ADR-0031-chrome-extension-first-capture.md) — 操作記録とresponsive captureの正本。
...
13. [テスト戦略](07-quality/test-strategy.md) — Fast / Core / Deep Gate。
...
- `03-architecture`: システム構成、Chrome拡張capture、認証、手順書モデル、ADR。Browser Runは将来／legacy安全契約
- `04-data`: ERD、D1テーブル・Worker認可境界、R2、データライフサイクル
- `05-api`: API契約、guest bootstrap/claim、Product Event、Webhook
- `06-security`: セキュリティ、プライバシー、脅威モデル
- `07-quality`: テスト戦略、MVP受入条件、レビュー基準
...
- 対象のFR/NFRが記載されている。
- 関連API/データ/UI/テストの正本が必要な範囲で存在する。
- P0/P1扱いの未決事項がない。
- `issue-map.md` に実装単位と完了条件がある。
- Deferred機能は、別機能の安全境界に必要な場合を除き先行実装しない。

## 現行MVPのProduct最優先

`guest -> Chrome拡張 -> PC/スマホ/タブレット -> local draft -> output時signup -> Personal Workspace bootstrap -> guest claim -> output完了`

Cloudflare Access/D1/R2は認証後の安全なサーバー基盤として利用しますが、Cloudflare移行マイルストーン完了数だけをProduct進捗やPMFの根拠にはしません。

Browser Run / Browser Session / Live Viewは現行MVPのcapture runtimeでは使用しません。
...
Cloudflare移行は認証後の安全なサーバー基盤のために継続します。Product最優先はIssue #227のguest-first Chrome拡張MVPです。
