...
`capture.browserRun.egressVerified.enabled=false` の場合も、認証、workspace role、tenant entitlement、request envelopeを確認する。同じ`operationKey`の既存reservationをfingerprint/state照合する例外はcapture-session createだけに限定し、同じkey・同じfingerprintのterminal retryは`200`、in-flightまたは`result_unknown` retryは`202 RESERVATION_RESULT_UNKNOWN`で同じreservation stateを返す。この既存retryではegress gate、quota、capacityを再評価せずBrowser Runへ通信しない。同じkeyでfingerprintが異なるcapture-session createは`409 RESERVATION_REQUEST_MISMATCH`で拒否する。live-url、commands、mobile preview sessionの各routeはcapture-startの`operationKey` lookupを行わず、既存egress fail-closed契約を維持する。新しいcapture session、live-url、commands、mobile preview session処理はBrowser Runへ通信する前に`503 BROWSER_EGRESS_NOT_VERIFIED`で拒否する。hostnameのallowlistや運営承認はこの拒否を迂回できず、Browser Runを起動する新しいAPIにも同じgateを必須にする。
