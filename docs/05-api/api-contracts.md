...
`capture.browserRun.egressVerified.enabled=false` の場合も、認証、workspace role、tenant entitlement、request envelopeを確認する。同じ`operationKey`の既存reservationをfingerprint/state照合する例外はcapture-session createだけに限定し、同じkey・同じfingerprintのterminal retryは`200`、in-flightまたは`result_unknown` retryは`202 RESERVATION_RESULT_UNKNOWN`で同じreservation stateを返す。この既存retryではegress gate、quota、capacityを再評価せずBrowser Runへ通信しない。同じkeyでfingerprintが異なるcapture-session createは`409 RESERVATION_REQUEST_MISMATCH`で拒否する。mobile preview session createはcapture-startとは別の`mobilePreviewOperationKey`とroute固有reservationを持ち、同じkey・同じfingerprintのterminal retry=200、in-flight/`result_unknown` retry=202、異なるfingerprint=409とし、新規だけatomicな時間・同時実行数reservationとegress gateを通過させる。live-url、commandsはcapture-startの`operationKey` lookupを行わず、mobile preview navigate/reloadは別契約がAcceptedになるまで未実装・fail closedとする。新しいcapture session、live-url、commands、mobile preview session処理はBrowser Runへ通信する前に`503 BROWSER_EGRESS_NOT_VERIFIED`で拒否する。hostnameのallowlistや運営承認はこの拒否を迂回できず、Browser Runを起動する新しいAPIにも同じgateを必須にする。
