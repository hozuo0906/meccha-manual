...
以下のSignup Conversion、Capture Completion／Failure、Output Gate ConversionおよびTTFVをserver-side Product Eventから算出する場合、対象は認証後にprivacy-safe eventをflushしたcreator cohortである。LP／Chrome Web Store等のanonymous aggregateはapplication Product Eventとは別指標として扱い、未認証の全guestを観測できるとはみなさない。

### Signup Conversion

分母は新規アカウント作成導線で記録した`signup_started`とし、returning userのloginを含めない。

分子は対応するserver-sideのidentity新規作成で一度だけ記録した`signup_completed`とする。clientの自己申告やbootstrapの再送回数を新規signup数として数えない。

### Activation Rate

分母: `output_gate_opened` に到達したguest cohort。returning userや既存の認証済みcreatorを含めない。
...
`draft_generated`に対する`output_gate_opened`の割合。server-sideでは認証後に両eventをflushしたguest cohortだけを対象とする。
