...
| AC-018 | StripeまたはDiscordの正規provider requestがAccess JWTなしでexact callback pathへ到達する | exact POSTとbody上限を確認し、raw bodyのprovider署名・署名対象timestampを副作用なしで検証してから有界parse/schema検証を行い、provider event/interaction IDをauthoritative storeへ原子的に予約する | 新規予約成功後だけQueue、外部API、業務D1、entitlementその他の副作用へ進み、Access user、service token、D1 identity、workspace membershipへ写像しない。`Origin`をcallback認証に使わない |
| AC-019 | 別method、subpath、body超過、署名欠落・不正・期限外、payload不正・provider ID欠落、replay、または通常アプリAPI／`GET /health/config`へのAccessなしrequest | callback境界を通過しようとする | method/path/body/署名/timestamp違反はparse前、payload/ID違反は予約前、replayは原子的予約時に拒否する。予約以外のQueue、外部API、業務D1、entitlementその他の副作用を起こさない。hostname全体やwildcard pathへBypassを適用せず、通常アプリAPIはAccess user、`GET /health/config`はservice tokenを必須にする |
