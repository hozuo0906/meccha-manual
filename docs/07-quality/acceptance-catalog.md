...
| AC-009 | active identityかつactive membershipの有効ownerが1人だけ存在する | そのownerを停止、削除、owner以外へ変更する、またはidentityをdisabledへ変更する | APIとDBで拒否し、ワークスペースに有効ownerを1人以上維持する。disabled identityのowner membershipは代替ownerに数えない |
...
| AC-018-M2 | M2のStripe/Discord exact callback POST（署名の有無、body不正、env未設定を含む） | callback停止境界へ到達する | body読取、Origin／署名処理、KV／Queue／D1、外部fetch、`waitUntil`、成功ackを行わず、両入口で安定した `503 CALLBACK_MIGRATION_IN_PROGRESS` を返す。path別Access BypassはOFFを維持し、M5では副作用0をnegative proofする。 |
