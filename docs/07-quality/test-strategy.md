...
- PDFはMVPとして、`local draft -> PDF選択 -> output gate -> signup/login -> bootstrap -> claim -> PDF出力再開 -> 完了`を確認する。signupキャンセルではlocal draftを保持してeditorへ戻し、claim成功確認前はlocal原本を削除しない。Markdown／HTML exportはNEXTとしてPDFのMVP gateへ混在させない。
...
- Primary TTFVは`capture_started -> draft_generated`を維持する。認証後にflushしたcreator cohortのTTFV／Capture Completion／Failureは診断できるが、Full Guest Signup Conversion、Full Guest Activation Rate、Full Guest Output Gate Conversionは現行server-side Product Eventでは算出不可であり、post-auth cohort診断を全guest指標として表示しない。
...
- maximized／fullscreenを含むoriginal window id／bounds／stateを保存し、normalへのtransition後にtarget outer boundsを適用してinner sizeを補正する。orientation切替、記録停止、取消、例外、target tab close、service worker suspension後のいずれでもoriginal bounds／stateへ戻る。
...
- R2 put成功／D1 commit結果不明を再現し、決定的object keyでD1/R2をreconcileして同一digestを再利用する。digest mismatchをfail closedにし、incomplete orphanだけをcleanupし、completed claimのassetを削除せず、manual／assetを二重生成しない。
