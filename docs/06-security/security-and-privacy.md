...
- claim assetのobject keyはclaimIntentId、operationId、server-authoritative asset slotから決定的に導出し、retryで別keyを作らない。R2 put成功／D1結果不明では再upload前に両状態をreconcileし、同一digestだけを再利用する。mismatchはfail closedとする。
- incomplete claimのorphan staged assetだけをcleanup対象とし、D1 completed claimが参照するassetは削除しない。R2とD1を単一transactionとはみなさない。
...
- window変更前のid、left、top、width、height、stateをservice worker suspension後も読めるactive capture sessionへ保存する。maximized／fullscreen等はnormalへtransition後にtarget boundsを適用し、終了・取消・例外・target tab close時にoriginal boundsとstateの両方を復元する。
