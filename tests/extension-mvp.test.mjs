...
test("masking failure cannot fall back to an unmasked screenshot", async () => {
  let captured = false;
  await assert.rejects(captureWithMaskBoundary({ applyMasks: async () => ({ applied: false }), capture: async () => { captured = true; return "data:image/jpeg;base64,AA"; }, removeMasks: async () => {} }), /SCREENSHOT_MASK_FAILED/);
...
test("recorder stop removes click, input, change and scroll listeners and timer", async () => {
  const source = await readFile(new URL("../apps/extension/content/recorder.js", import.meta.url), "utf8");
  for (const event of ["click", "input", "change", "scroll"]) assert.match(source, new RegExp(`removeEventListener\\("${event}"`));
