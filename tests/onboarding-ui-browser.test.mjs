...
test("onboarding browser retries the same operation after a 503 and rejects expired reload metadata", { timeout: 20_000 }, async () => {
  ...
  try {
    ...
    const firstCreatedAt = await page.evaluate(() => JSON.parse(sessionStorage.getItem("meccha-manual:onboarding-operation")).createdAt);

    await page.goto(`${baseUrl}/onboarding/continue?same=1#handoff=${handoff}`);
    assert.equal(await page.evaluate(() => JSON.parse(sessionStorage.getItem("meccha-manual:onboarding-operation")).createdAt), firstCreatedAt);
    await page.reload();
...
test("onboarding does not mint or retry an expired operation after a failed request", { timeout: 20_000 }, async () => {
  ...
  try {
    ...
    const initialOperation = await page.evaluate(() => JSON.parse(sessionStorage.getItem("meccha-manual:onboarding-operation")).operationId);
    ...
    assert.equal(calls.length, 0);
    assert.equal(await page.evaluate(() => JSON.parse(sessionStorage.getItem("meccha-manual:onboarding-operation")).operationId), initialOperation);

    await page.goto(`${baseUrl}/onboarding/continue?retry=1#handoff=${handoff}`);
    const firstOperation = await page.evaluate(() => JSON.parse(sessionStorage.getItem("meccha-manual:onboarding-operation")).operationId);
    await page.locator("#bootstrap").click();
    ...
    await page.waitForFunction(() => document.querySelector("#bootstrap")?.disabled === true);
    assert.equal(calls.length, 1);
    assert.equal(await page.evaluate(() => JSON.parse(sessionStorage.getItem("meccha-manual:onboarding-operation")).operationId), firstOperation);

    await page.evaluate(() => {
      const value = JSON.parse(sessionStorage.getItem("meccha-manual:onboarding-operation"));
      value.createdAt = new Date(Date.now() - 32 * 60 * 1000).toISOString();
      sessionStorage.setItem("meccha-manual:onboarding-operation", JSON.stringify(value));
    });
